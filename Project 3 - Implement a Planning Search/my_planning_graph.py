...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        for action in self.all_actions:
            ...
            add=True
            # verifies that all the preconditions of an action are met, if they are in the previous state
            for n in Node.prenodes:
                if n not in self.s_levels[level]:
                    add=False
                    break
            # if all the preconditions are met
            if add:
                #address node to a_levels
                self.a_levels[level].add(Node)
                #Adjust the connection between the action node and the child nodes
                for n in Node.effnodes:
                    Node.children.add(n)
                #Adjust the link between the action node and the parents nodes
                for n in Node.prenodes:
                    Node.parents.add(n)
    ...
    def add_literal_level(self, level):
        ...
        for a1 in self.a_levels[level-1]:
            for s1 in a1.effnodes:
                self.s_levels[level].add(s1)
                s1.parents.add(a1)
            for s2 in a1.prenodes:
                s2.children.add(a1)
        ...
        for g in self.problem.goal:
            ...
            while level<len(self.s_levels):
                for s in self.s_levels[level]:
                    if s.symbol==g and not find and s.is_pos:
                        find=True
                        break
                if find:
                    break
